...
#CHANGE ME
base_dir = "/home/spot-vision/Documents/RotationDisamb/"
...
correct_csv_output_path = os.path.join(new_dir, f"{test_name}_CORRECTED.csv")
...
print("Corrected CSV Output Path:", correct_csv_output_path)
    ...
    while True:
        ...
        if stream_selector_ready:
            if best_stream_idx == 0:
                correct_pose_output.append([current_time-start_time, *candidates[0][0], *candidates[0][1]])
            if best_stream_idx == 1:
                correct_pose_output.append([current_time-start_time, *candidates[1][0], *candidates[1][1]]) 
        ...
        try:
            ...
            Rx = normal[0]   # x component of normal (cos angle wrt camera optical axis)
            Rz = normal[2]   # z component of normal (cos angle wrt camera optical axis)

            data = bytearray(struct.pack("fff", Tx, Tz, Rz))
            sock.sendto(data, server_address)

            print(f"[POSE] Frame {frames} | Using Pose {idx} | Tx={Tx:.2f}, Rx={Rx:.3f} Tz={Tz:.2f}, Rz={Rz:.3f}")
...
with open(correct_csv_output_path, mode="w", newline="") as f:
    writer = csv.writer(f)
    writer.writerow(correct_header)
    writer.writerows(correct_pose_output)
