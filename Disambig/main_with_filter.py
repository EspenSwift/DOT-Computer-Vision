...
from temporal_filter import TemporalPoseFilter
import csv
import socket
import struct
import os
...
#       TEMPORAL FILTER
# ==========================================================
temporal_filter = TemporalPoseFilter(w_pos=1.0, w_ang=0.7)
temporal_initialized = False
...
try:    
    while True:
        ...
        if not stream_selector_ready:
            ...
            if not candidates_are_dummy(candidates):
                ...
                if len(Rx1_buf) >= STREAM_WINDOW_STEPS + 1:
                    ...
                    # Initialize temporal filter using the correct stream
                    init_center = candidates[best_stream_idx][0]
                    init_normal = candidates[best_stream_idx][1]
                    temporal_filter.initialize(init_center, init_normal)
                    temporal_initialized = True

                    print("[temporal] Initialized filter with pose stream:", best_stream_idx)
        ...
        try:
            # ----------------------------------------------------------
            # Choose best pose using temporal filtering (after warm-up)
            # ----------------------------------------------------------

            if stream_selector_ready and not candidates_are_dummy(candidates):

                # Use constant dt for now (10 FPS)
                dt = 1.0 / FPS

                # Format candidates as (p, n) pairs for the filter
                cand_list = [
                ...
                if temporal_initialized:
                    # Let temporal filter choose best candidate
                    idx, best_p, best_n, dbg = temporal_filter.step(cand_list, dt)
                else:
                    # Should not occur, but fallback
                    idx = best_stream_idx
            else:
                # If temporal filter not ready or candidate is dummy, fallback
            ...
            # ----------------------------------------------------------
            # Extract chosen pose and send over UDP
            # ----------------------------------------------------------
            center = candidates[idx][0]   # (Rx, Ry, Rz)
            normal = candidates[idx][1]   # (Tx, Ty, Tz) direction cosines
