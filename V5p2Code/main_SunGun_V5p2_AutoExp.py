...
import time
from EllipseFittingFunctions_SunGun_V5p2_AutoExp import *
...
# Format the date and time into a string suitable for a filename (YYYY-MM-DD_HH-MM-SS)
# Colons are replaced with hyphens or underscores for compatibility
date_string = now.strftime("%B-%d_%H-%M")

# Construct the full file name
test_name = date_string#input("Please enter the test name: ")
...
zed.set_camera_settings(sl.VIDEO_SETTINGS.EXPOSURE, 35)   # 0–100
...
# Slight warm tone
zed.set_camera_settings(sl.VIDEO_SETTINGS.WHITEBALANCE_TEMPERATURE, 5000)
...
#HARDCODING CAMERA INTRINSICS - USING FRAME DIMENSIONS AND TAKING CENTRE PIXEL AS PRINCIPAL POINT, AND USING ZED CALIBRATION DATA TO GET FOCAL LENGTH IN PIXELS, THEN CONVERTING TO MM BASED ON ZED PIXEL SIZE
#This is because there seems to be some issue with the way the ZED intrinsics are beingread in - they are coming in with incorrect values for focal length and principal point, which is causing issues with the pose estimation. By hardcoding the intrinsics based on the known camera parameters, we can ensure that the pose estimation is working correctly. The focal length in pixels is calculated based on the
# Build intrinsic matrix K
"""
K_left = np.array([
    [left.fx, 0.0, w/2],
...
try:    
    while True:
        ...
        if prev_mean_intensity > 0:
            
            if prev_mean_intensity < 125:
                # increase exposure to brighten image
                
                new_exposure = min(current_exposure + 3, 95)  # increase by 5, cap at 100
            ...
            elif prev_mean_intensity > 150:
                # decrease exposure to darken image
           
                #current_gain = zed.get_camera_settings(sl.VIDEO_SETTINGS.GAIN)[1]
                new_exposure = max(current_exposure - 3, 15)  # decrease by 5, floor at 5
        ...
        if chosen is not None:
            ...
            if abs(Tz)> 4 or abs(Tx) > 4 or abs(theta_tc) > 3.9 or abs(theta_tc) < 2.3:
        ...
        if raw_writer is not None and write_videos:
            ...
            if ellipse is not None and write_videos and send_flag ==1:
        ...
        try:
            ...
            # Sanity check - remove any poses that are obviously wrong
            ...
            print(f"Flag: {send_flag}\t\tX_tc: {round(100*Tz,1)} cm\t\tY_tc: {-round(100*(Tx-0.03),1)} cm\t\t"f"Theta: {round(theta_tc,2)} rad\t\tInliers: {round(inlier_frac,2)}\t\tIntensity: {round(prev_mean_intensity)}\t\tExposure: {round(current_exposure)}")
