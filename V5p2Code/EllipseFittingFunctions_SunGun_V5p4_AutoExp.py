...
def detect_outer_circle(frame):
    ...
    gray_blur = cv2.GaussianBlur(gray, (15,15), 2)
...
def EllipseFromFrame(frame_bgr, prev_max_area, prev_Tz_history):
    ...
    if prev_max_area > MIN_AREA_HOUGH_CIRCLE and outer_circle is not None:
        ...
        r = int(r*1.1)  # slightly expand radius to ensure we capture the whole region
    ...
    if not use_hough_circle:
        gold_pixels = gray_frame[gold_mask == 255]
        if len(gold_pixels) > 0:
            avg_intensity = float(np.mean(gold_pixels))
        else:
            avg_intensity = 0.0
