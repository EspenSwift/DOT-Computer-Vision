...
def get_gold_mask(frame_bgr, kernel_size = 7, iterations = 3):
    ...
    # SET WHICH COLOUR MASK TO USE (DEPENDING ON LIGHTING CONDITIONS OBSERVED)
    mask = mask_YELLOW
    
    mask = cv2.morphologyEx(mask,
                            cv2.MORPH_CLOSE,cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (kernel_size, kernel_size)),iterations=iterations)
...
def EllipseFromFrame(frame_bgr, prev_max_area, prev_circle, used_prev_circle):
    ...
    if send_circle:
        ...
        if r_old is not None:
            cv2.circle(circle_mask, (cx, cy), r_old, 255, -1)
        else:
            cv2.circle(circle_mask, (cx, cy), r, 255, -1)
        ...
        if max_contour is not None:
            ...
            if ellipse is not None and (AR < MAX_AR) and inlier_frac > MIN_INLIER_FRAC:
                ...
                used_prev_circle = 0
                prev_circle = outer_circle
                return ellipse, prev_max_area, prev_circle, used_prev_circle, avg_intensity
            else:
                return None, 0, prev_circle, used_prev_circle, avg_intensity
        else:
            return None, 0, prev_circle, used_prev_circle, avg_intensity
    else:
        return None, 0, None, 0, avg_intensity