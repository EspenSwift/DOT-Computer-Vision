...
import time
from EllipseFittingFunctions_SunGun_AutoExp import *
...
test_name = datetime.now().strftime("%m%d_%H%M")
...
# Initialize previous circle and counter (Hough Outer Circle)
prev_circle = None
used_prev_circle = 0
...
try:    
    while True:
        # First, check if previous ellipe fit was valid to tweak exposure for next frame
        if send_flag == 1:
        ...
        ellipse, prev_max_area, prev_circle, used_prev_circle, prev_mean_intensity = EllipseFromFrame(frame_bgr, prev_max_area, prev_circle, used_prev_circle, prev_mean_intensity)
        ...
        try:
            ...
            if abs(Tz)> 4 or abs(Tx) > 4 or abs(theta_tc) > 3.6 or abs(theta_tc) < 2.5:
