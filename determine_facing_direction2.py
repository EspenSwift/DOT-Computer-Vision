...
def determine_facing(img, ellipse):
    ...
    #Ellipse reference logic
    ellipse_center_x = ellipse[0][0]
    left_side = ellipse_center_x < w / 2   # whether ellipse is on left side of frame

    if box_poly is not None and len(box_poly) >= 3:
        ordered = order_by_y(box_poly)

        if left_side:
            side_points = sorted(ordered[-2:], key=lambda p: p[0]) 
        else:
            side_points = sorted(ordered[-2:], key=lambda p: p[0], reverse=True)
        ...
        if lines is not None:
            ...
            for l in lines:
                ...
                # Skip lines from the opposite side of ellipse
                if left_side and (x1 > w/2 and x2 > w/2):
                    continue
                if not left_side and (x1 < w/2 and x2 < w/2):
                    continue
